...
class Execute:
    ...
    def start():
        ...
        if not sap_crd_param:
            ...
            sap_crd:str = str(sap_crd_param)
            
        sharepointfolder_path_param = execution.parameters.get('sharepointfolder_path')
        if not sharepointfolder_path_param:
            raise ValueError("Parâmetro 'sharepointfolder_path' não encontrado nos parâmetros da execução.")
        else:
            sharepointfolder_path:str = str(sharepointfolder_path_param)
        
        
        ExecuteAPP(
            user=maestro.get_credential(label=sap_crd, key="user"), 
            password=maestro.get_credential(label=sap_crd, key="password"), 
            ambiente=maestro.get_credential(label=sap_crd, key="ambiente")
        ).start(
            destino=SharePointFolders(sharepointfolder_path, paths_register_json_path=os.path.join(f"C:\\Users\\{os.getlogin()}",'paths_register.json')).value
        )
